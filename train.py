...
from algorithm.dataset import TestCleanDataset,TrainCleanDataset, TrainprocessDataset, TestprocessDataset
from algorithm.dataset_multifeature import TrainCleanDataset_multifeature, TestCleanDataset_multifeature
from algorithm.fusion.model import MultiDiff, save2file
...
def get_params():
    ...
    # model
    parser.add_argument("--epsilon_theta", type=str, default='EnhanceUGnet')
    ...
    parser.add_argument("--T_h", type=int, default=100)
    parser.add_argument("--T_p", type=int, default=80)
...
def default_config(data='SN'):
    ...
    config.data.feature_file = config.data.path + config.data.name + '/normalized_flow.npy'
    config.data.test_file = config.data.path + config.data.name + '/normalized_flow_test.npy'
    config.data.spatial = config.data.path + config.data.name + '/adj.npy'
    config.data.num_recent = 1

    if config.data.name == 'SN':
        config.data.num_features = 4
        config.data.num_vertices = 5
        config.data.points_per_hour = 12
        config.data.val_start_idx = int(2832 * 0.9)
        config.data.test_start_idx = int(2832 * 0.95)

    if config.data.name == "GAIA":  # the same as AIOPS
        config.data.num_features = 11
        config.data.num_vertices = 54
        config.data.points_per_hour = 12
        config.data.val_start_idx = int(8760 * 0.6)
        config.data.test_start_idx = int(8760 * 0.8)

    if config.data.name == 'MMS':
        config.data.num_features = 16
        config.data.num_vertices = 211
        config.data.points_per_hour = 12
    ...
    config.model.epsilon_theta = 'EnhanceUGnet'
    ...
    # training config
    config.model_name = 'FUSION'
    config.is_test = False  # Whether run the code in the test mode
    config.epoch = 250  # Number of max training epoch
    config.optimizer = "adam"
    config.lr = 1e-3
    config.batch_size = 32
    config.wd = 1e-5
    config.early_stop = 30
...
def save_batches(y_true, y_pred, batch_index):
    """
    Save y_true and y_pred as .npy files with names starting from 1.
    
    :param y_true: numpy array of true values
    :param y_pred: numpy array of predicted values
    :param batch_index: index of the current batch (starting from 0)
    """
    # File names start from 1
    y_true_filename = f'data/dataset/SN/predict_data/y_true_batch_{batch_index + 1}.npy'
    y_pred_filename = f'data/dataset/SN/predict_data/y_pred_batch_{batch_index + 1}.npy'
    
    # Save the arrays
    np.save(y_true_filename, y_true)
...
def evals(model, data_loader, epoch, metric, config, clean_data, mode='Test'):
    ...
    for i, batch in enumerate(data_loader):
        ...
        x = x.transpose(1, 3).float()  # (B, F, V, T)
        x_masked = x_masked.transpose(1, 3).float()  # (B, F, V, T)
        ...
        # x_hat=clean_data.reverse_sample_normalization(x_hat)
        # x_hat = x_hat.detach()
        ...
        _y_true_ = f_x.transpose(1, 3).cpu().numpy()  # y_true: (B, T_p, V, F)
        _y_pred_ = f_x_hat.transpose(2, 4).cpu().numpy() # y_pred: (B, n_samples, T_p, V, D)
        save_batches(_y_true_, _y_pred_, i)
        
        # _y_true_ = f_x.transpose(0, 3, 2, 1)  # (B, T_p, V, F)
        # _y_pred_ = f_x_hat.transpose(0, 1, 4, 3, 2)  # (B, n_samples, T_p, V, F)
        ...
        # _y_true_ = h_x.transpose(0, 3, 2, 1)
        # _y_pred_ = h_x_hat.transpose(0, 1, 4, 3, 2)
        ...
        batch_metrics.append({
            'batch': i + 1,
            'future_metrics': metrics_future.metrics,
            'history_metrics': metrics_history.metrics
        })
...
def main(params: dict):
    ...
    # clean_data = TrainCleanDataset(config)
    clean_data = TrainCleanDataset_multifeature(config)
    
    # test_clean_data = TestCleanDataset(config)
    test_clean_data = TestCleanDataset_multifeature(config)
    config.model.A = clean_data.adj

    model = MultiDiff(config.model)
    model = model.to(config.device)

    # Load training dataset
    # train_dataset = TrafficDataset(clean_data, (0 + config.model.T_p, config.data.val_start_idx - config.model.T_p + 1), config)
    train_dataset = TrainprocessDataset(clean_data, (0 + config.model.T_h, clean_data.feature.shape[0] - config.model.T_p + 1), config)
    ...
    test_dataset = TestprocessDataset(test_clean_data, (0+ config.model.T_h, test_clean_data.feature.shape[0] - config.model.T_p + 1), config)
...
# SN	UGnet	200	    UGnet	            32	        12	        12	        ddpm
