...
from .enhance_ugnet import EnhanceUGnet
...
class MultiDiff(nn.Module):
    ...
    def __init__(self, config: easydict):
        ...
        elif config.epsilon_theta == 'EnhanceUGnet':
            self.eps_model = EnhanceUGnet(config).to(self.device)
...
def save2file(params):
    file_name = ws + f'/output/metrics/MultiDiff.csv'
    head = [
        # data setting
        'data.name',
        # mdoel parameters
