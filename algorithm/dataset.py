...
class TestCleanDataset(Dataset):
    def __init__(self, config):
        self.dataset_test = np.expand_dims(np.load(config.data.test_file)[:, :, 0], -1)
        self.t_h = config.model.T_h
        self.t_p = config.model.T_p
    ...
    def combine_segments(self):
        ...
        for i, segment in enumerate(self.normalized_segments):
    ...
    def __len__(self):
        return len(self.normalized_segments)

    def __getitem__(self, idx):
        return self.normalized_segments[idx]
...
class TestprocessDataset(Dataset):
    ...
    def get_idx_lst(self):
        ...
        idx_list = list(range(start, end, self.segment_length))
        idx_lst = []
        for idx in idx_list:
